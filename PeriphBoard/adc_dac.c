...
void enable_adc_clk(void){
    enable_clock(
        16u,    // PM_APBCMASK enable is in the 16 position
        0x17    // ID for ADC 0x17 (see table 14-2)
        );
}
...
void enable_dac_clk(void){
    enable_clock(
        18u,    // PM_APBCMASK enable is in the 18 position
        0x1A    // ID for DAC 0x1A (see table 14-2)
        );
}
